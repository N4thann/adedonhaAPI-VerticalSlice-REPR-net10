...
- **Indexes are created at startup**, not via migrations: `Shared/Data/MongoDbIndexService` is an `IHostedService` that creates unique indexes on `Category.Slug` and `Word.Slug`, plus compound indexes on `Word.Categories.Slug` (category detail listing) and `Word.InitialLetter + Word.Categories.Slug` (the "10 random words per letter/category" catalog view). New query patterns should generally get a matching index added here rather than relying on collection scans.
